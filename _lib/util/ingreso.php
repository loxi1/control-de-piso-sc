...
function verificarRegistroIngreso(array $param): ?array {
    $conf = EnvConfig::getMySQL();
    $conn = PDOp::mysql($conf['mes']);

    if (!$conn || empty($param)) return [];
...
function existencia_ciclos(array $p): int {
    $conf = EnvConfig::getMySQL();
    $conn = PDOp::mysql($conf['mes']);
...
function mostrarTurno(int $empresa): ?array {
    $conf = EnvConfig::getMySQL();
    $conn = PDOp::mysql($conf['mes']);
    if (!$conn || empty($empresa)) return [];
