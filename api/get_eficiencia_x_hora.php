...
// ⏰ Obtener el tiempo total transcurrido en minutos
...
$tiempo_total_min =  {rs_data_sybase}[0][0];

// ⚙ ⚙️ Obtener eficiencia
$sql = "SELECT
    co.operacion,
...
if (isset({rs_data_sybase}[0][0])) {
    foreach ({rs_data_sybase} as $row) {
        ...
        $valorobtenido = $tiempo_estandar*$cant*100;
        $eficiencia += $valorobtenido;
    }
}

$eficiencia = $eficiencia == 0 ? 0 : number_format(($eficiencia*60/$tiempo_total_min), 2, '.', '');
